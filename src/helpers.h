...
string hasData(string s) 
{
    ...
    {
        string::size_type b1 = s.find_first_of("[");
        string::size_type b2 = s.find_first_of("}");
...
size_t WhichLane(double d)
{
    return (size_t)std::round(std::max(0.0, (d-2)/4));
}

typedef std::vector<HwyCoord> HwyTrajectory;
